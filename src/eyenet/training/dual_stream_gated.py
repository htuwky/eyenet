...
from eyenet.models.dual_stream import DualStreamGatedAttentionModel
from eyenet.training.baseline import compute_metrics
from eyenet.training.dual_stream_concat import (
    DualStreamConcatConfig,
    build_dual_dataset,
    build_threshold_map,
    evaluate,
    train_one_epoch,
...
def train_fixed_split(
    ...
    model = DualStreamGatedAttentionModel(
    ...
    for split_name, labels, probabilities, subject_ids in [
        ...
        for threshold_name, threshold in threshold_map.items():
            ...
            metric_rows.append(
                {
                    "model": "dual_stream_gated_attention",
                    "split": split_name,
...
def collect_attention_weights(model, loader, device: str, split_name: str) -> list[dict]:
    ...
    for batch in loader:
        ...
        macro_gate = attention["macro_gate"].cpu().numpy()
                ...
                rows.append(
                    {
                        ...
                        "macro_gate": float(macro_gate[row_idx]),
                        "event_gate": float(1.0 - macro_gate[row_idx]),
