...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run EMS macro + pretrained encoder dual-stream fusion over seeds.")
    parser.add_argument("--seeds", default="0,1,2,3,4")
    parser.add_argument("--fusions", default="gated,concat")
    parser.add_argument("--split-dir", default="data/splits/EMS/multiseed")
    parser.add_argument("--output-root", default="experiments/ems_encoder_dual_stream")
...
def parse_items(value: str) -> list[str]:
    ...
    unknown = sorted(set(items) - {"concat", "gated"})
    if unknown:
        raise ValueError(f"Unknown fusions: {unknown}. Choices: concat,gated")
    return items
...
def run_one(args: argparse.Namespace, seed: int, fusion: str) -> None:
    ...
    command = [
        sys.executable,
        "scripts/train_ems_encoder_dual_stream_fixed_split.py",
        "--macro-segments",
        args.macro_segments,
        "--encoder-events",
        args.encoder_events,
